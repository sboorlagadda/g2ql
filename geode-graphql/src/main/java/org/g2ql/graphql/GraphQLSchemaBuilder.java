...
public class GraphQLSchemaBuilder extends GraphQLSchema.Builder {
  ...
  GraphQLObjectType getQueryType() {
    GraphQLObjectType.Builder queryType = GraphQLObjectType.newObject().name("QueryType_Geode")
  ...
  GraphQLFieldDefinition getQueryFieldDefinition(String regionName,
    ...
    try {
      ...
      return GraphQLFieldDefinition.newFieldDefinition().name(regionName)
      ...
      arguments.addAll(Arrays.stream(regionAttributes.getValueConstraint().getDeclaredFields())
          ...
          .map(this::getArgumentForField).collect(Collectors.toList()));
  ...
  GraphQLFieldDefinition getCollectionTypeQueryFieldDefinition(String regionName,
    ...
    try {
      ...
      return GraphQLFieldDefinition.newFieldDefinition().name(regionName + "s")
          .description(getSchemaDocumentation(regionAttributes.getValueConstraint()))
      ...
      arguments.addAll(Arrays.stream(regionAttributes.getValueConstraint().getDeclaredFields())
          .filter(this::isGraphQLArgument).filter(f -> isBasicAttributeType(f.getType()))
          .map(this::getListArgumentForField).collect(Collectors.toList()));
  ...
  GraphQLType getObjectType(String regionName, Class<?> valueClass) {
    ...
    List<GraphQLFieldDefinition> fields = Arrays.stream(valueClass.getDeclaredFields())
        .filter(f -> !isConnectionType(f)).map(this::getObjectField).collect(Collectors.toList());

    // connection type
    fields.addAll(Arrays.stream(valueClass.getDeclaredFields()).filter(f -> isConnectionType(f))
        .map(f -> getConnectionType(regionName, f)).collect(Collectors.toList()));
  ...
  private GraphQLArgument getArgument(Class<?> key) {
    ...
    return GraphQLArgument.newArgument().name("key") // key is the argument for any type in geode
  ...
  private GraphQLArgument getArgumentForField(Field field) {
    ...
    return GraphQLArgument.newArgument().name(field.getName()).type((GraphQLInputType) type)
        .build();
  }

  private GraphQLArgument getListArgumentForField(Field field) {
    GraphQLType type = getBasicAttributeType(field.getType());
    return GraphQLArgument.newArgument().name(field.getName()).type(new GraphQLList(type)).build();
  }

  private GraphQLFieldDefinition getObjectField(Field field) {
    try {
      GraphQLType type = getBasicAttributeType(field.getType());
      return GraphQLFieldDefinition.newFieldDefinition().name(field.getName())
          .description(getSchemaDocumentation(field)).type((GraphQLOutputType) type).build();
    } catch (UnsupportedOperationException ex) {
      // the field is non-java, so go deep
      GraphQLObjectType embedded = GraphQLObjectType.newObject()
          .name(field.getType().getSimpleName()).description(getSchemaDocumentation(field))
          ...
          .build();
      return GraphQLFieldDefinition.newFieldDefinition().name(field.getName()).type(embedded)
          .build();
    }
  }
  ...
  private GraphQLFieldDefinition getConnectionType(String regionName, Field field) {
    ...
    GraphQLFieldDefinition connectionDefinition = GraphQLFieldDefinition.newFieldDefinition()
        .name(field.getName()).description(getSchemaDocumentation(field)).type(connectionType)
